...
The cost of producing large quantities of sequence data has greatly decreased with the improvement in both the chemistry and algorithm development for processing sequence data. With short read *de novo* assemblies, although genomes with a high level of coverage could be produced, because of the length of short reads, assemblies using short reads alone remained very fragmented. This is because there are certain regions of a genome that are hard for short read sequences to assemble through. These are mainly sequence features that span greater than the length of a short read. Typically these contain repeat sequences. Long read assemblies allow the reads to span these difficult regions in these genomes. However with long read assemblies the error rate is higher so we introduce SNPs, and we have lower coverage. We covered this previously in our other course in the episode on [Platform choice](https://cloud-span.github.io/experimental_design01-principles/01-platform/index.html), and in the [introduction section of this course](https://cloud-span.github.io/metagenomics01-qc-assembly/00-introduction-meta/index.html). However one way in which we can both reduce the fragmentation of the genome and remove some of these SNPs, is through polishing.

## Why bother polishing?

Depending on your downstream use, polishing your assembly may not be essential. However commonly one of the first questions we ask of our metagenome is often, what is in here? In order to answer this question, which we will cover in [Taxonomic annotations](https://cloud-span.github.io/metagenomics03-taxonomic-anno/).
