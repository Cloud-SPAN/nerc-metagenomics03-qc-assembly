...
* Finally we have two files for a repeat graph (`assembly_graph.{gfa|gv}`) which is a visual way to view the assembly - we come back to this in a bonus exercise later.    
...
> If you are interested you can explore the repeat graph created by Flye.
> In order to do this we need to install a program called Bandage. You can download this from the [Bandage Website](https://rrwick.github.io/Bandage/) by selecting your operating system down the side.
> Once the software is installed, we also need to download the graph files generated by the assembler. To do this, follow the steps for `scp` onto your local computer.
> 
{: .callout}
